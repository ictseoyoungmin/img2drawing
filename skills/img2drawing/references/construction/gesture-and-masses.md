# Gesture and masses

Construction states a spatial hypothesis for the subject before local description.

## Gesture

Use the dominant action to connect head, torso, pelvis, support, and major limb movement.
The line of action is a directional summary, not a centerline that every body part must
follow.

This construction leaf is a capability, not by itself a user-facing finish definition. When the
requested final mode is gesture drawing, read `../modes/gesture-drawing.md`: pure gesture and
constructive gesture have explicit whole-pose completion criteria. A few torso masses, cross-axes,
or a line of action do not authorize completion while major visible limb/support relations are
missing. When gesture is only the first pass of a larger drawing request, continue into that larger
mode after the structural hypothesis is credible.

## Masses

Place head, ribcage, and pelvis as occupied forms with the observed tilt, width, depth,
and overlap. Generic ellipses, beans, or boxes are acceptable only as temporary reasoning
when they still preserve those observed relationships; they are not the target shape.

Do not stop at placement. A mass also has orientation: turn relative to the viewer, near/far
side, projected centerline or cross-axis, visible plane exposure, and a relation to the next
mass. When placement looks plausible but the pose still feels flat, frontal, or symmetric,
read `orientation-and-twist.md` before refining contour.

Keep masses connected by the actual pose. Do not infer a complete hidden torso or limb
through an occluder merely to make construction feel anatomically complete.

A successful construction already reads as this subject's pose at a glance. If it does not,
repair the premise before adding contour, identity accents, or broad value.
