...
def _explicit_profile(identity: tuple[str, str], *, width: int = 160, height: int = 80) -> RenderProfile:
    ...
    return RenderProfile(
        profile_id=f"test-{renderer_id}-{renderer_version}", renderer_id=renderer_id,
        renderer_version=renderer_version, canvas_width=width, canvas_height=height,
        supersample=4,
    )
...
def _draw_broad(session: DrawingSession, policy: str, *, stroke_id: str = "broad") -> None:
    ...
    session.draw(
        ((16, 40), (58, 36), (104, 43), (144, 39)), stroke_id=stroke_id,
        part="material-probe", pressure=(1.0, 1.0, 1.0, 1.0), **mark.draw_kwargs(),
    )
...
def test_v103_renderer_identities_remain_registered_for_explicit_replay() -> None:
...
def test_checkpoint_header_tracks_profile_and_v9_replays_after_resume(tmp_path: Path) -> None:
    ...
    session = DrawingSession.create(
        subject=subject, output_dir=tmp_path / "v9-run", render_profile=_explicit_profile(V9)
    )
    session.draw(
        ((18, 42), (80, 34), (142, 42)), stroke_id="legacy-line", part="legacy",
        tool="form_pencil", tool_overrides={"width": 2.4, "pressure": 0.72, "opacity": 0.80},
    )
...
def test_explicit_v10_remains_available_for_rc1_rc2_replay(tmp_path: Path) -> None:
    subject = _subject(tmp_path)
    session = DrawingSession.create(
        subject=subject, output_dir=tmp_path / "v10-run", render_profile=_explicit_profile(V10)
    )
...
def test_explicit_v11_remains_v103_material_policy_authority(tmp_path: Path) -> None:
    subject = _subject(tmp_path)
    means: dict[str, float] = {}
    hashes: dict[str, str] = {}
    for policy in ("canonical-pencil", "manga-light", "dry-graphite-expressive"):
        session = DrawingSession.create(
            subject=subject, output_dir=tmp_path / policy, render_profile=_explicit_profile(V11)
        )
...
def test_explicit_v11_fast_final_matches_canonical(tmp_path: Path) -> None:
    subject = _subject(tmp_path)
    session = DrawingSession.create(
        subject=subject, output_dir=tmp_path / "fast-run", render_profile=_explicit_profile(V11)
    )
    high = resolve_markmaking(
        "pencil_loose", "contour", modifiers={"width": 6.2, "pressure": 0.82, "opacity": 0.90}
    )
    low = resolve_markmaking(
        "pencil_loose", "form", modifiers={"width": 7.0, "pressure": 0.68, "opacity": 0.78}
    )
