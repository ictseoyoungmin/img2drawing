...
GRAPH CLEANUP:      Slice A authority cleanup ACTIVE · no drawing/runtime semantic change
...
- **Instruction-graph attention cleanup is maintenance work around the same bottleneck, not a replacement product bottleneck.** Slice A only aligns documentation authority and records the later B–E plan; it does not add drawing rules or change runtime behavior.
...
```text
Slice A authority synchronization                         ACTIVE
Slice B SKILL.md router reduction + direct quality gate   BLOCKED by A
...
The detailed scope and closure rules live in `INSTRUCTION_GRAPH_ATTENTION_ARCHITECTURE_PLAN.md`. Do not broaden Slice A into prose reduction or drawing-rule changes.
