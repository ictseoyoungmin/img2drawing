...
B07에서 evidence budget과 cost telemetry를 닫고, B08에서 orthogonal plain-data
DrawingIntent/mode/style scaffolding을 닫았다. 다음은 B09 Mode-aware finish이다.
```
...
4. [`slices/`](slices/) — 앞으로 실행할 B09–B18 작업 카드와 최근 closure record
...
- B08의 `DrawingIntent`, `ModeGuide`, `StyleGuide`는 public plain-data API이며,
  lifecycle cursor나 renderer pipeline이 아니다. `RenderProfile`은 이후 slice다.
